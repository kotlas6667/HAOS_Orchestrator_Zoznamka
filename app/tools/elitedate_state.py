...
_STATE_FILE = Path(__file__).resolve().parent.parent.parent / "elitedate_state.json"
...
def resolve_current(chosen_text: str) -> dict[str, Any] | None:
    """Pop the head of the queue after it has been sent, promote the next one."""
    state = _load()
    if not state["queue"]:
        return None

    sent_entry = state["queue"].pop(0)
    sent_entry["status"] = "sent"
    sent_entry["chosen_text"] = chosen_text
