...
- **Morning greet (ED, optional):** when `MORNING_GREET_ENABLED=true` / HA `morning_greet_enabled`, daily at 07:00 `morning_greet.py` opens `/ucet/novi-clenove`, applies the search filter if needed, walks up to `morning_greet_max_profiles` unique profiles, opens „Napísať správu“, and sends `Ahoj :-)` only when the thread has no history. Processed IDs live in `.morning_greeted.json` (anti-loop). Debug: `POST /debug/morning_greet`.
