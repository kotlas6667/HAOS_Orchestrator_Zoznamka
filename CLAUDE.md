...
A second, independent process — `elitedate_bot/` — automates a personal Elite Date account (a dating site with no public API) via Selenium, and hands the "which reply to send" decision to the user over Discord. It is designed to run alongside the orchestrator on the same Raspberry Pi and talks to it over localhost HTTP in both directions (no polling of the orchestrator needed):
...
- Runs as its own systemd service (see `elitedate_bot/elitedate-bot.service.example`), separate from the orchestrator's — a Selenium crash shouldn't take down the main app.
