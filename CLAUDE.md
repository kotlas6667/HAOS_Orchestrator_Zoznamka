...
- **Orchestrator side:** `app/tools/elitedate_dispatch.handle_incoming()` generates two alternative AI reply drafts via GPT (`app/tools/elitedate_reply_provider.py`), using HA setting `elitedate_reply_skill` (or sidecar `/data/orchestrator/config/elitedate_reply_skill.md`, else bundled `elitedate_reply_skill.md`), stores them in a FIFO queue on disk (`elitedate_state.json`, via `app/tools/elitedate_state.py` — only the head of the queue is `awaiting_selection` at any time, to keep a bare "1"/"2" Discord reply unambiguous), and posts the two options to Discord.
